...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            z_t=tf.math.sigmoid(tf.matmul(inputs,W_z)+tf.matmul(state,U_z)+b_z)
            r_t = tf.math.sigmoid(tf.matmul(inputs, W_r) + tf.matmul(state, U_r) + b_r)

            h_new_t=tf.math.tanh(tf.matmul(inputs,W_o)+tf.multiply(r_t,tf.matmul(state,U_o))+b_o)

            h_t=tf.multiply(z_t,state)+tf.multiply((1-z_t),h_new_t)
