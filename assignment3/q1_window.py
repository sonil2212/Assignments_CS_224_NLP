...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for index in range(window_size, window_size + len(labels)):
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder=tf.placeholder(tf.int32,shape=(None,),name="label_placeholder")
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        if labels_batch is not None:
            labels_batch = (labels_batch[:, None] == np.arange(labels_batch.max() + 1)).astype(int)
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (!3-5 lines)
        emd_tensor=self.pretrained_embeddings
        embeddings=tf.nn.embedding_lookup(emd_tensor,self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        h=tf.nn.relu(tf.math.add(tf.matmul(x,W),b1))

        h_drop = tf.nn.dropout(h, dropout_rate)

        pred = tf.math.add(tf.matmul(h_drop,U),b2)
    ...
    def add_loss_op(self, pred):
        ...
        print "\n"
        print tf.shape(self.labels_placeholder)
        print tf.shape(pred)
        # print tf.shape(tf.transpose(self.labels_placeholder))
