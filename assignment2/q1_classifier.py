...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        #decalre weights and biases
        weights=tf.Variable(initial_value=tf.zeros((self.config.n_features,self.config.n_classes),tf.float32))
        biases=tf.Variable(initial_value=tf.zeros((self.config.n_classes,),dtype=tf.float32))

        #calculate predictions
        mul=tf.matmul(self.input_placeholder,weights)
        temp=tf.add(mul,biases)
